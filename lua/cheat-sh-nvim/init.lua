...
local function openTerminal(command)
	local command_string = "terminal curl https://cht.sh/" .. command
...
local function openWindow(isCommand)
	...
	local window = createWindow(bufferHandler, width, height, widthRatio, heightRatio, CursorRowOffset, CursorColOffset,
		windowType, anchor)

	-- Makes new windows follow same theme as the editor
	vim.api.nvim_win_set_option(window, 'winhl', 'Normal:Myhighlight')
	-- vim.api.nvim_set_option_value('winhl',{Normal = window}, {win =0})
end
